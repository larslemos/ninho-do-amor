...
export default function JHWeddingReligious({ isOpen, onClose }: JHWeddingReligiousProps) {
  const religiousRef = useRef<HTMLDivElement>(null);

  const handleClickOutside = (event: MouseEvent) => {
    if (religiousRef.current && !religiousRef.current.contains(event.target as Node)) {
      onClose();
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      ...
      >
        ...
        <div className="overflow-x-auto text-left">
          ...
          <p className="wedding-text-primary font-fira-sans mb-2 space-y-6 text-sm sm:text-base">
            <MapPin className="inline h-4 w-4 sm:h-5 sm:w-5" /> Igreja Universal do Reino de Deus -
            Mantendene
          </p>
          ...
          <ul className="wedding-text-primary font-fira-sans mb-10 space-y-6 text-sm sm:text-base">
            <li className="flex items-center">
              <Church className="mr-2 h-4 w-4 sm:h-5 sm:w-5" /> Entrada dos padrinhos
            </li>
            <li className="flex items-center">
            ...
            <li className="flex items-center">
              <Heart className="mr-2 h-4 w-4 sm:h-5 sm:w-5" /> Palavra e bênção do pastor
            </li>
            <li className="flex items-center">
              <Heart className="mr-2 h-4 w-4 sm:h-5 sm:w-5" /> Troca de votos e alianças
            </li>
            <li className="flex items-center">
              <Pen className="mr-2 h-4 w-4 sm:h-5 sm:w-5" /> Assinatura dos documentos
            </li>
            <li className="flex items-center">
              <Heart className="mr-2 h-4 w-4 sm:h-5 sm:w-5" /> Bênção final e saída dos noivos
            </li>
          </ul>
