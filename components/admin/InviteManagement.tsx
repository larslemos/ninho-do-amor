...
export default function InviteManagement() {
    ...
    const getConfirmSentStatus = (guest: Guest) => {
        return guest.confirm_sent_count > 0 ? `✅ x${guest.confirm_sent_count}` : '❌';
    };
    ...
    const getWhatsAppDeliveredStatus = (guest: Guest) => {
        return guest.whatsapp_delivered_count > 0 ? `✅ x${guest.whatsapp_delivered_count}` : '❌';
    };
    ...
    const getSmsDeliveredStatus = (guest: Guest) => {
        return guest.sms_delivered_count > 0 ? `✅ x${guest.sms_delivered_count}` : '❌';
    };
    ...
    return (
        <div className="p-6">
            ...
            <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-5">
                <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 place-card">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm font-medium text-blue-600 font-josefin">Total Convidados</p>
                            <p className="text-2xl font-bold text-blue-900 font-quicksand">{stats.total}</p>
                        </div>
                        <Users className="h-8 w-8 text-blue-600" />
                    </div>
                </div>

                <div className="rounded-lg border border-green-200 bg-green-50 p-4 place-card">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm font-medium text-green-600 font-josefin">Convites WhatsApp</p>
                            <p className="text-2xl font-bold text-green-900 font-quicksand">{stats.whatsappSent}</p>
                        </div>
                        <MessageCircle className="h-8 w-8 text-green-600" />
                    </div>
                </div>

                <div className="rounded-lg border border-purple-200 bg-purple-50 p-4 place-card">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm font-medium text-purple-600 font-josefin">Confirmações Manuais</p>
                            <p className="text-2xl font-bold text-purple-900 font-quicksand">{stats.manualConfirmations}</p>
                        </div>
                        <CheckSquare className="h-8 w-8 text-purple-600" />
                    </div>
                </div>

                <div className="rounded-lg border border-orange-200 bg-orange-50 p-4 place-card">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm font-medium text-orange-600 font-josefin">Lembretes Enviados</p>
                            <p className="text-2xl font-bold text-orange-900 font-quicksand">{stats.remindersSent}</p>
                        </div>
                        <Bell className="h-8 w-8 text-orange-600" />
                    </div>
                </div>

                <div className="rounded-lg border border-teal-200 bg-teal-50 p-4 place-card">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm font-medium text-teal-600 font-josefin">Confirmados</p>
                            <p className="text-2xl font-bold text-teal-900 font-quicksand">{stats.confirmed}</p>
                        </div>
                                ...
                                <tr key={guest.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                        <div className="flex gap-1">
                                            ...
                                            <button
                                                onClick={() => handleInvitationAction(guest.id, 'markSmsSent')}
                                                disabled={sendingAction === guest.id}
                                            ...
                                            <button
                                                onClick={() => handleInvitationAction(guest.id, 'markSmsDelivered')}
                                                disabled={sendingAction === guest.id}
