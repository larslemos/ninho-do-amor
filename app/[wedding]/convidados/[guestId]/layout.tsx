...
export default async function WeddingLayout({
  ...
  if (!weddingData || !weddingData.design_elements || !weddingData.design_elements.branding) {
    console.error('Wedding data not found or invalid:', {
      wedding,
      weddingData,
    });
    return <div className="font-blancha">Erro: Dados do casamento não encontrados</div>; // Improved fallback with Blancha font
  }
  ...
  return (
    <AudioProvider>
      <AudioControl />
      {/* Layout UI */}
      <main className={`font-blancha ${weddingData.design_elements.branding}-theme`}>
        {children}
      </main>
    </AudioProvider>
  );
