...
export async function GET(request: NextRequest) {
  try {
    ...
    // Verify guest exists
    const { data: guest, error: guestError } = await supabase.from("guests").select("id").eq("token", token).single()
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!token || !name || !message) {
      return NextResponse.json({ error: "Token, nome e mensagem são obrigatórios" }, { status: 400 })
    }
